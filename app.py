...
def is_playable(card_orbs,resource_orbs):
    is_playable = True
    card_count = color_count_dict(card_orbs)

    for key,value in card_count.items():
        if color_count_dict(resource_orbs[:len(card_orbs)])[key] < value:
            is_playable = False
    return is_playable
...
df_playable = df[df['is_playable']==True]
...
st.dataframe(final_deck.sort_values(by=['orbsamount'])[['name','faction','type','orbsamount']], width=600, height=550)
